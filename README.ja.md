...
v0.2はリリース済みです。3スキルはピン留めしたGitHub Releaseからインストールでき、StandaloneのTest Assessment ModeとCI・自動Release Pipelineが利用可能です。現在のSourceには必須Host Adapter Review-only Entry Point、Fail-closed Isolation Gate、厳格な実行Artifact検証、Mutation Report v8、正準4ブロックRendererを追加しています。Standalone Mutation実行はBlockedで、信頼されたHostがRun Nonce、保護された外部Storage、Repository全体のRead-onlyまたはWrite Monitor Capabilityを発行する必要があります。
