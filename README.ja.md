...
v0.3.0 Integration Previewでは、Claude Code TerminalをPrimary Hostとします。Plugin Archiveを展開し、Claudeを直接起動せず、`python3 ./socratic-plugin-v0.3.0-alpha.2/scripts/claude_host.py --primary /path/to/repository`でBundled Hostから起動してください。LauncherはRepositoryを外部の使い捨てWorkspaceへCopyし、Live Unix socket Host brokerを開始してPluginをLoadします。直接の`claude --plugin-dir`は引き続きfail-closedです。
...
Maieutic・Elenchus開発用のStandalone Agent Skillも利用できますが、`$socratic`だけをInstallしてもPre-agent Safety Boundaryにはなりません。

```bash
# 対話式でStandalone Skillを選ぶ
gh skill install Shogo1222/socratic

# Standalone Skillをすべてまとめてインストール
