...
gh skill install Shogo1222/socratic --all --pin v0.4.0-alpha.3
```
...
v0.4 Prototypeでは、機械的なMutation実行をAgent InstructionからNarrow Runnerへ移す。確定したDecisionと型付きTest Profile境界は[Narrow Runner Architecture Decision](docs/ja/runner-architecture.md)と[Narrow Runner Test Profile](docs/ja/test-profiles.md)に記載する。`local-copy` Prototypeは型付きPython unittest Experimentを実行して未署名Evidenceを生成できる。正準Reviewについては現行Host-gated Runtimeを引き続き正とする。
...
v0.2はリリース済みです。3スキルはピン留めしたGitHub Releaseからインストールでき、StandaloneのTest Assessment ModeとCI・自動Release Pipelineが利用可能です。現在のSourceには必須Host Adapter Review-only Entry Point、Fail-closed Isolation Gate、厳格な実行Artifact検証、Mutation Report v10、正準4ブロックRendererを追加しています。Standalone Mutation実行はBlockedで、信頼されたHostがRun Nonce、保護された外部Storage、Repository全体のRead-onlyまたはWrite Monitor Capabilityを発行する必要があります。
