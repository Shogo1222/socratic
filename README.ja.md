...
# 公開Releaseへピン留め
gh skill install Shogo1222/socratic --all --pin v0.2.5
```
...
v0.2はリリース済みです。3スキルはピン留めしたGitHub Releaseからインストールでき、StandaloneのTest Assessment ModeとCI・自動Release Pipelineが利用可能です。現在のSourceにはFail-closed Isolation Gate、厳格な実行Artifact検証、Mutation Report v6、正準4ブロックRendererを追加しています。決定的な言語別AdapterとHost側Read-only Adapterは今後の実装対象です。
