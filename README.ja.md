...
Codex向けv0.3.0 Integration Previewでは、Standalone Socratic SkillではなくPluginをInstallしてください。Pluginは`UserPromptSubmit` Host Hookを同梱し、NativeなTrusted Host Adapterがない場合、明示的なSocratic RequestをAgent開始前に停止します。承認済みMarketplaceへEntryが公開された後、署名済みPlugin BundleをそのMarketplaceからInstallしてください。このSource Repositoryが個人・組織Marketplaceへ自身を自動登録することはありません。
