...
v0.2はリリース済みです。3スキルはピン留めしたGitHub Releaseからインストールでき、StandaloneのTest Assessment ModeとCI・自動Release Pipelineが利用可能です。現在のSourceには必須Review-only Entry Point、Fail-closed Isolation Gate、厳格な実行Artifact検証、Mutation Report v7、正準4ブロックRendererを追加しています。Mutation実行にはHost側Read-onlyまたはWrite MonitorのAttestationが必要です。
