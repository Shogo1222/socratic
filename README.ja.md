...
v0.3.0 Integration Previewでは、Claude Code TerminalをPrimary Hostとします。Plugin ArchiveをDownloadし、`claude --plugin-dir ./socratic-plugin-v0.3.0-alpha.2.zip`で起動してください。`UserPromptSubmit` Hookは、信頼されたMutation境界がない場合、Claudeが明示的なSocratic Requestを処理する前に停止します。Archiveには実験的なCodex Manifestも残します。
