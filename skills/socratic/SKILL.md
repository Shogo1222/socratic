...
When the user supplies a GitHub pull-request URL or `PR #<number>` in the invocation prompt, use only the Host-materialized review root from the injected command. The Host, not this Skill, resolves and fetches exact Base and Head commits, verifies their SHAs, and records them as `change_context`. Never fetch the PR again, substitute the caller's current checkout, or claim a different PR provenance. If Host materialization fails, the gate is terminally blocked.
