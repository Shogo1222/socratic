...
Ephemeral must hold on every exit path. `finish` keeps Host artifacts only while the structured disposition question is pending. After the answer, or when it is unanswered, invoke the dedicated `cleanup` operation; treat no answer as Discard. Failure, timeout, interruption, and abort invoke the same idempotent cleanup immediately. If cleanup fails, report only the exact remaining paths.
...
The Host Adapter issues the run ID, nonce, protected storage root, artifact index, and repository-wide protection evidence. `preflight_with_host` validates every run path outside Primary, records the initial Primary content hash, creates the preparation workspace and isolated cache/temp/home directories, writes the manifest once, and starts a nonce-bound append-only ledger. Run dependency installation and the successful baseline only in `prepared_root`. Before the first mutation, the Runner seals that state with a content hash. Write fixed `challenge-plan.json` under `artifact_root` and call `challenge-batch`; the Runner validates unique IDs, argv, timeouts, and bounded parallelism, creates clones in plan order, executes tests concurrently, and records results in plan order. Classify only after all raw outcomes return. Each `mutate` or `register_prebuilt` creates a fresh Mutation-ID sandbox from the sealed snapshot, preferring a Host-detected copy-on-write clone and recording an explicit `full-copy` fallback when unavailable. Never install dependencies separately in each mutant. Keep each clone's HOME, cache, temp, and package-manager cache isolated. Keep the Runner-owned raw outcome (`passed`, `failed`, or `timeout`) separate from the inference-owned `outcome_interpretation`. `finish` rechecks prepared-snapshot integrity, verifies classification claims against the Ledger, and injects prepared-snapshot, clone-strategy, raw execution, Host, isolation, postflight, and renderer evidence into Mutation Report v10.

The Host context also provides one session-specific `artifact_root`. For canonical runs, write only the fixed Draft files `intent-contract.draft.json`, `mutation-report.draft.json`, and `canonical-review.draft.json` directly under that directory. The Mutation Report Draft contains analysis fields only and must not contain run identity, attestation, isolation, postflight, or renderer hash fields. Call `stage-artifact` once for each Draft; it performs strict schema validation and records a create-once hash in the Host-managed artifact index. Except for the experimental Plan described below, the tool gate rejects every other filename, Primary, Sandbox, Manifest, Ledger, arbitrary temporary paths, traversal, and symlinks. After `finish`, resolve artifact and proven-test disposition through structured UI, then call `cleanup`.
...
The canonical surface remains four blocks. Before completion, stage the three fixed Drafts and invoke `scripts/run_review.py finish`; do not call the lower-level validator as a substitute. The finish entrypoint generates the attested Mutation Report from Host evidence, validates the run manifest, guarded-write ledger, Intent Contract, generated Report, and canonical review object, then delegates to `scripts/validate_and_render.py`. Treat its stdout as the complete reviewer-facing result; do not translate, summarize, or append prose. Render retained JSON artifacts only with the bundled strict JSON renderer. Parse failure, schema failure, missing or mismatched run identity, unknown Contract references, an unsafe Review-only postflight, or prose outside the renderer output blocks completion. Present proven-test disposition afterward through the host's structured question UI, not as a fifth review block; use the three-option Markdown fallback only when structured questions are unavailable, then invoke `cleanup`.
