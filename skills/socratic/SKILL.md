...
Every Review-only mutation run must use the trusted Host Adapter integration for `preflight_with_host`, each guarded `mutate` or `register_prebuilt`, a baseline `execute`, every mutation-specific `execute`, and `finish`. The standalone CLI cannot create a ready run and always returns `blocked`; self-asserted attestation JSON is never accepted. If the Host Adapter, schema, or verified host read-only/write-monitor capability is unavailable, stop before running any mutation. Manual approximation, direct Primary mutation followed by restoration, repository commands outside `execute`, hand-written artifacts, and hand-written four-block output are non-compliant and must never be presented as a Socratic run.
