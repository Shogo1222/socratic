...
After `scaffold-analysis` and before invoking `complete`, summarize the classified outcomes and ask through the structured question tool how to keep the artifacts:
...
Ephemeral must hold on every exit path. Retention is decided before `complete` because `complete` renders the final surface and performs the default cleanup in the same step; a question asked after rendering can no longer save anything. Treat no answer as Discard and use that default cleanup. Use `--retention keep` only for an explicit Save locally or Markdown choice. Failure, timeout, interruption, and abort invoke the same idempotent cleanup immediately. If cleanup fails, report only the exact remaining paths.
...
After the canonical review surface, ask one structured operational question: **Apply tests**, **Output patch**, or **Discard**. Do not offer Apply tests for an unresolved mapped oracle. Selecting Apply tests is the explicit authorization required to continue the same cycle in Apply tests mode. The run-artifact retention question is a different checkpoint: the handoff outlives `complete`, but run artifacts do not, so retention is resolved before `complete` (see Run artifacts). Treat no answer as Discard.
...
Use local Git only for strictly read-only evidence gathering. Allowed commands are limited to `git diff`, `git show`, `git log`, `git rev-parse`, `git merge-base`, and `git ls-files`; `git archive` is not read-only (`-o` writes a file) and the Host gate denies it. During an active hook-host run, prefix each with `git --no-pager`; add `--no-ext-diff --no-textconv` to `diff`, `show`, and `log`. Prefer a host-provided diff or already-materialized Base and Head snapshots when available.
...
Ask the human to confirm or correct the recommendation. Treat the recommendation as routing, never as specification evidence. If the invocation already explicitly selects one of these types, restate it and ask for confirmation rather than asking an open-ended scope question. In a non-interactive session where no answer can arrive, proceed with the recommended Review Type and record in the analysis that it was not human-confirmed.
...
Every Review-only mutation run must use the trusted Host Adapter integration and the fixed Runner pipeline: `preflight`, `runbook` once, structured `inspect`, `scaffold-contract`, `stage-artifact`, optional `execute --phase prepare`, one successful `probe-command`, `scaffold-plan`, one `challenge-batch`, `scaffold-analysis`, then `complete`. This pipeline is the canonical orchestration contract: where a Maieutic or Elenchus document describes a different mutation mechanism, that description applies only to their standalone use, never to a Socratic-invoked run. Execute every Runner command synchronously in the foreground and wait for it to finish: never launch `prepare`, `probe-command`, `challenge-batch`, or `complete` as a background task, and never end a turn to wait for one — in a non-interactive session an ended turn kills the background process and voids the run. The standalone CLI cannot create a ready run and always returns `blocked`; self-asserted attestation JSON is never accepted. If the Host Adapter, schema, or Host-attested read-only/write-monitor capability is unavailable, stop before running any mutation. In schema v10, `verified: true` means that the Runner accepted the trusted Host's attestation, not that the Runner independently verified the OS boundary. Manual approximation, direct Primary mutation followed by restoration, repository commands outside the Runner, hand-written attestation fields, hand-written complete reports, and hand-written four-block output are non-compliant and must never be presented as a Socratic run.

Immediately after preflight, run the `runbook` command from the preflight result's `next.argv` exactly once: it explains the ID glossary (DEC/INV/FX/UNR/CMD/MUT), the gate order, the fields you may edit, and returns the exact next command. Every successful Runner result carries `next.argv` — run it verbatim, replacing only the `<angle-bracket>` placeholders; never guess or reorder arguments. Every scaffold result also carries `editable_fields` and a Runner-owned `field_guide` with required keys, allowed enum values, conditional rules, and schema-valid element templates; use that guide instead of opening a schema or guessing a shape. Before the first step, present the runbook's execution plan to the user in their language as a short numbered list. Announce each phase transition as a human goal ("verifying accident patterns"), never as an internal command name; if one phase exceeds 30 seconds, say why and report elapsed time, and never show an unfounded percentage.

Use the injected Host review context as the deterministic fast path. Do not launch subagents to rediscover the Base/Head diff, package manager, or fixed pipeline, and do not retry package-manager wrappers when the direct focused test executable is already identified. Infer intent, call `scaffold-contract`, replace every semantic placeholder in the returned document from repository evidence, and use `Write` exactly once to create the returned absolute `artifact_path`. Stage it, and only then submit challenges whose `contract_ids` name the observable oracle. The Runner rejects mutation before a staged Contract and blocks every challenge mapped to an unresolved oracle. Ask the structured question, record the answer with accurate provenance, restage in a fresh run, and continue only after that oracle is resolved.
...
The Host Adapter issues the run ID, nonce, protected storage root, artifact index, and repository-wide protection evidence. Use `inspect diff|file|search|tests` for bounded read-only evidence instead of shell discovery or deterministic subagents. Run dependency installation once with `execute --phase prepare`. Before assigning any Mutation ID, call `probe-command` with the exact focused test argv; it branches a fresh source clone, detects TTY, package-manager, cwd, timeout, and dependency failures as infrastructure, and records a reusable command ID only on success. After the successful probe, the Runner seals `node_modules` and recognized Python virtual environments as a separately hashed shared dependency layer; each mutant keeps its own HOME, temp, and cache directories. Probe the direct test executable — locate it in the prepared snapshot first with `inspect search`; in a hoisted workspace this is typically `../../node_modules/.bin/vitest run <test file>` with `--cwd packages/<package>` — instead of a package-manager wrapper such as `pnpm exec` or `npm run`: wrappers may re-evaluate or reinstall dependencies, while the direct binary runs immediately against the sealed layer. The recorded command ID reuses the probed argv and `--cwd` for every batch execution. If the probe returns `blocked`, read its bounded stdout, stderr, and any `hint`, correct the focused command or its cwd, and rerun the probe through its returned `next.argv`; never proceed to `scaffold-plan` without a validated command ID. Sandbox commands run with the project's own toolchain: the injected runtime Python exists only for `run_review.py`, so use the project's interpreter or test binary by absolute path when version requirements matter. The prepared snapshot contains no `.git` — VCS-version build backends rely on the preset `SETUPTOOLS_SCM_PRETEND_VERSION`, and versioningit needs its project-side `default-version` — and HOME is redirected into the sandbox, so HOME-based tool shims such as uv, nvm, or pyenv may not resolve; prefer absolute tool paths. After an infrastructure failure, run the read-only `doctor` command from the result's `diagnose.argv` before changing tools or arguments: it reports the sandbox PATH, the toolchain executables that resolve there with their versions, the project's interpreter and build-backend requirements, the VCS-metadata situation, and the last failed command — never probe the environment with ad-hoc shell instead.

Write one compact `challenge-plan.json` containing the validated command ID, intent-linked accident metadata, and only `replace-exact` or `delete-exact` anchored edits. Never embed a complete source file, command argv per mutant, hashes, or ledger data. The Runner validates every anchor against the prepared source before creating any mutant, seals the source snapshot, creates one copy-on-write source clone per Mutation ID, attaches the sealed dependency layer, applies edits itself, runs the validated command in parallel, and returns raw outcomes in plan order. Classify only after all raw outcomes return. Keep Runner-owned raw outcomes separate from inference-owned interpretation.

The Host context provides one session-specific `artifact_root`. The agent may write only `intent-contract.draft.json`, `challenge-plan.json`, `review-analysis.json`, and — on the experimental prototype path only — `experiment-plan.json` there, and every canonical document starts from a Runner scaffold: `scaffold-contract` before staging the Contract, `scaffold-plan` after the successful probe (it binds the validated command ID), and `scaffold-analysis` after `challenge-batch`. Each scaffold command returns a schema-valid `document` and its absolute `artifact_path` without creating the target. Replace only the semantic placeholders, then use `Write` exactly once to create that path. If correction is needed after creation, `Read` the file and use `Edit`; never retry `Write` on an existing scaffold. Never reconstruct these documents from scratch or read schema files to construct them. After `challenge-batch`, call `scaffold-analysis`; the Runner joins the Plan to raw outcomes in the returned analysis document. Edit only intent, classification rationale, detecting tests, follow-up, and reviewer-facing claims. A failed process starts as `inconclusive`, never automatically `killed`. The document contains no baseline mechanics, mutation implementation, run identity, attestation, isolation, postflight, hashes, or ledger data. Call `complete` once after classification. The Runner validates the semantic input, generates and stages the Report and Review Drafts, cross-checks every classification against raw evidence, renders the canonical surface, and performs cleanup. A `complete` failure is terminal and cleans the disposable run; report it and start a fresh run rather than retrying the deleted Manifest. Use `--retention keep` only after an explicit artifact-retention choice; otherwise use the default `discard`.
...
Require isolated execution, a stable baseline, one fresh sandbox per attributable mutant — executed concurrently by the Runner's validated `challenge-batch`, never by interleaving edits in a shared workspace — explicit `not_challenged` items, postflight proof that no production mutation remains, and a validated handoff when Review-only proves a proposed test.
