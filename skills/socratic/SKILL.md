...
The Host Adapter issues the run ID, nonce, protected storage root, artifact index, and repository-wide protection evidence. `preflight_with_host` validates every run path outside Primary, records the initial Primary content hash, creates the preparation workspace and isolated cache/temp/home directories, writes the manifest once, and starts a nonce-bound append-only ledger. Run dependency installation and the successful baseline only in `prepared_root`. Before the first mutation, the Runner seals that state with a content hash. Each `mutate` or `register_prebuilt` creates a fresh Mutation-ID sandbox from the sealed snapshot, preferring a Host-detected copy-on-write clone and recording an explicit `full-copy` fallback when unavailable. Never install dependencies separately in each mutant. Keep each clone's HOME, cache, temp, and package-manager cache isolated. Keep the Runner-owned raw outcome (`passed`, `failed`, or `timeout`) separate from the inference-owned `outcome_interpretation`. `finish` rechecks prepared-snapshot integrity, verifies classification claims against the Ledger, and injects prepared-snapshot, clone-strategy, raw execution, Host, isolation, postflight, and renderer evidence into Mutation Report v9.
