...
For the v0.3.0 integration preview, Claude Code Terminal is the primary Host. Extract the Plugin archive, then launch through the bundled Host instead of invoking Claude directly: `python3 ./socratic-plugin-v0.3.0-alpha.3/scripts/claude_host.py --primary /path/to/repository`. The launcher copies the repository to an external disposable workspace, starts a live Unix-socket Host broker, and loads the Plugin. Direct `claude --plugin-dir` remains fail-closed.
