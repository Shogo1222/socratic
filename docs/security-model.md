...
<!-- socratic-distribution-file-count: 43 -->
<!-- socratic-plugin-file-count: 63 -->
The standalone Skill distribution contains exactly 43 UTF-8 text files under the three skill directories, including sixteen bundled Python source files (four script entrypoints and the twelve-module runner package behind `run_review.py`), the current run schemas, and the experimental Plan, Evidence, and Interpretation schemas. The audited multi-Host Plugin component set contains exactly 63 UTF-8 text files: those Skills plus Claude Code, Codex, and Cursor manifests, marketplaces, Host hooks, the shared broker, and the Plugin-managed Python runtime bootstrap. Claude Marketplace uses the repository root as its source, so its marketplace checkout can also contain repository files outside this audited runtime component set, such as `demo/`, `docs/`, and `site/`; the 63-file claim describes the audited Plugin bundle and release asset, not every file materialized by that source checkout. The Python files have no POSIX execute bits but are run by a Python interpreter. CI's executable-file rejection checks the POSIX `0o111` execute-bit mask; it also rejects unexpected files within the audited component set, unsupported extensions, symbolic links, binaries, and unapproved external hosts. Release assets include a manifest and SHA-256 checksums.
...
Dependency preparation and mutation execution use different filesystem roles. Baseline commands install dependencies once. After the successful probe, the Runner moves installed packages and recognized Python virtual environments into one Runner-owned dependency layer and seals that layer with its own content hash. Each Node sandbox receives a shallow local `node_modules` directory whose installed entries link to the shared layer; runtime outputs such as `node_modules/.vite` are therefore private to that sandbox rather than mutating the sealed packages. Every Mutation ID also receives private HOME, temp, and cache directories. Source staleness checks therefore traverse only source and configuration, while `finish` independently rejects either a changed source snapshot or changed installed package content. The Runner still prefers APFS clone or Linux reflink copy-on-write for source and records `full-copy` when neither is available.
...
Each live Host session creates one private `artifact_root` under Host storage. Write tools may create only the three fixed semantic inputs directly below that root: `intent-contract.draft.json`, `challenge-plan.json`, and `review-analysis.json`. For each input, the Runner returns a schema-valid document and its absolute fixed path without pre-creating the file; the agent may create it with one `Write`, then must use `Read` followed by `Edit` for corrections. The Contract is strictly validated and create-once hashed by `stage-artifact`; the Plan and Analysis are validated at their Runner boundaries. Only the Runner creates and stages the Report and Review Drafts. Those drafts cannot supply run identity, raw execution evidence, attestation, isolation, postflight, or renderer claims. `finish` derives those facts from the trusted Manifest and append-only Ledger, validates the resulting Mutation Report v10 and cross-artifact references before rendering, and writes only renderer stdout to the terminal. Paths in Primary, the disposable Sandbox, Manifest or Ledger locations, arbitrary temporary directories, other repositories, user configuration, and Plugin code remain denied. Successful runs retain Host artifacts only until the user resolves their disposition; `cleanup` then removes them. A terminal `finish` failure cleans the Sandbox, artifacts, index, Ledger, and Manifest immediately and must not be retried with the deleted Manifest.
...
Executing a repository's tests is running that repository's code with the invoking user's operating-system privileges. The Runner constrains what the agent may do and detects Primary tampering after the fact, but it does not itself provide OS-level containment: a malicious test suite can reach the network, read files the user can read, write outside the sandbox by absolute path, exhaust memory or disk, or leave processes behind. Use Socratic only on repositories you already trust to run locally — the same trust required to run their test suite by hand. Safe execution of unknown or adversarial PR code additionally requires host- or OS-level sandboxing (read-only mounts, network egress restrictions, resource limits), which this preview does not supply.
