...
The standalone Skill distribution contains 21 UTF-8 text files, including three Python source helpers. The multi-Host Plugin bundle contains 40 UTF-8 text files across Claude Code, Codex, and local Cursor Desktop integration. Those helpers have no POSIX execute bits but are run by a Python interpreter. The distribution audit's “executable” rejection specifically checks the POSIX `0o111` execute-bit mask. The distribution contains no binaries or symbolic links. Compare the preview with the release manifest and checksums.
