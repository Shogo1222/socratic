...
Review-only mutation execution has one normative entrypoint: `skills/socratic/scripts/run_review.py`. Its `preflight`, guarded `mutate`, and `finish` phases share a run identity through a validated manifest and write ledger. Without verified host read-only or repository-wide write-monitor attestation, preflight returns `blocked` and no mutation may run. Helper omission, Primary mutation followed by restoration, hand-written artifacts, and renderer-like prose are not valid protocol implementations.
