[English documentation](../protocol.md) | 日本語

# 日本語ドキュメント

英語版を正式仕様とし、日本語版は利用者およびメンテナーによる確認のために提供します。

## 共通設計

- [Intent Testing Protocol](protocol.md)
- [セキュリティモデル](security-model.md)
- [企業向け導入ガイド](enterprise-installation.md)
- [セキュリティポリシー](../../SECURITY.ja.md)

## Socratic

- [スキル本体](skills/socratic.md)

## Maieutic

- [スキル本体](skills/maieutic.md)
- [Intent Contract](skills/maieutic-intent-contract.md)
- [QA技法の選択](skills/maieutic-qa-techniques.md)

## Elenchus

- [スキル本体](skills/elenchus.md)
- [Intent駆動Mutation設計](skills/elenchus-mutation-design.md)
- [Mutation安全Contract](skills/elenchus-safety.md)
- [証明済みテストの引き渡し](skills/elenchus-test-handoff.md)
