...
Host ContextはSession固有の`artifact_root`も1つ提供する。Intent Contract、Mutation Report、Canonical Review JSONは、その正確なDirectory配下にだけ書き込む。Tool GateはPrimary、Sandbox、Manifest、Ledger、任意のTemporary Path、その他すべてのWrite Targetを拒否する。Artifact WriteはMutation Ledgerへ追記しないため、最後のGuarded Execution後に最終Ledger Headを含むReportを書き、その後に別の`execute`を挟まず`finish`を呼ぶ。
