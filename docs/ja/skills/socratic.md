...
すべてのReview-only Mutation Runは、信頼されたHost Adapterによる`preflight_with_host`、各`mutate`または`register_prebuilt`、BaselineとMutation ID付き`execute`、3つの検証済み`stage-artifact`、`finish`、`cleanup`を必ず使用する。Standalone CLIはReady Runを作れず、自己申告Attestation JSONを受理しない。Host Adapter、Schema、またはHostがAttestしたRead-only/Write-monitor CapabilityがなければMutation前に`blocked`で停止する。Schema v7の`verified: true`は、Runnerが信頼するHostのAttestationを受理したことを意味し、Runner自身がOS境界を独立検証したという意味ではない。手作業の近似、Primary変更後の復元、`execute`外のCommand、Attestation Fieldや完全Reportや4ブロックの手書きを正規Runとして提示しない。
...
Host AdapterはRun ID、Nonce、保護された外部Storage、Artifact Index、Repository全体の保護証跡を発行する。`preflight_with_host`は全Run PathがPrimary外であることを確認し、Primary初期Content Hashを記録し、Disposable Copyと隔離Cache／Temp／Homeを作成し、Manifestを一度だけ作成してNonce付きAppend-only Ledgerを開始する。`execute`はBaselineまたはMutation PhaseとMutation IDを記録し、各Report結果へ対応付ける。`finish`はPrimary最終Hashを確認してSandboxを破棄し、Host Identity、Nonce、Manifest Hash、Ledger Head、Isolation、Postflight、Canonical Output FactをMutation Report v7へ注入し、全SchemaとCross-artifact GateをRenderer前に検証して、Renderer stdoutだけをTerminalへ書く。
