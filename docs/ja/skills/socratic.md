...
すべてのReview-only Mutation Runは、信頼されたHost Adapterによる`preflight_with_host`、各`mutate`または`register_prebuilt`、BaselineとMutation ID付き`execute`、3つの検証済み`stage-artifact`、`finish`、`cleanup`を必ず使用する。Standalone CLIはReady Runを作れず、自己申告Attestation JSONを受理しない。Host Adapter、Schema、またはHostがAttestしたRead-only/Write-monitor CapabilityがなければMutation前に`blocked`で停止する。Schema v9の`verified: true`は、Runnerが信頼するHostのAttestationを受理したことを意味し、Runner自身がOS境界を独立検証したという意味ではない。手作業の近似、Primary変更後の復元、`execute`外のCommand、Attestation Fieldや完全Reportや4ブロックの手書きを正規Runとして提示しない。
...
Host AdapterはRun ID、Nonce、保護された外部Storage、Artifact Index、Repository全体の保護証跡を発行する。`preflight_with_host`は全Run PathがPrimary外であることを確認し、Primary初期Content Hashを記録し、Preparation Workspaceと隔離Cache／Temp／Homeを作成してNonce付きAppend-only Ledgerを開始する。依存導入と成功Baselineは`prepared_root`で一度だけ実行する。最初のMutation前にRunnerがContent Hashでその状態をSealする。各`mutate`または`register_prebuilt`はSeal済みSnapshotからMutation ID専用Sandboxを新規作成し、Hostで利用可能ならCopy-on-write、利用不能なら明示的な`full-copy` fallbackを記録する。Mutantごとの依存再導入は禁止し、HOME、Cache、Temp、Package-manager CacheもCloneごとに分離する。Runner所有のRaw Outcomeと推論側の`outcome_interpretation`を分離し、`finish`がPrepared Snapshot Integrity、分類、Clone Strategy、Host／Isolation／Postflight／Renderer EvidenceをMutation Report v9へ注入する。
