...
Host AdapterはRun ID、Nonce、保護された外部Storage、Artifact Index、Repository全体の保護証跡を発行する。Shell探索や決定論的Subagentの代わりに、`inspect diff|file|search|tests`で範囲を制限したRead-only Evidenceを取得する。依存導入は`execute --phase prepare`で一度だけ行う。Mutation IDを割り当てる前に、正確なFocused Test argvを`probe-command`へ渡す。RunnerはFresh Source Cloneで実行し、TTY、Package Manager、cwd、Timeout、Dependencyの問題をInfrastructure FailureとしてMutation前に検出し、成功時だけ再利用可能なCommand IDを記録する。Probe成功後、Runnerは`node_modules`と認識済みPython仮想環境を専用Hash付き共有Dependency LayerとしてSealし、各MutantのHOME、Temp、Cacheは個別に保つ。Probeには`pnpm exec`や`npm run`のようなPackage-manager Wrapperではなく、直接のTest Executableを使う——まず`inspect search`でPrepared Snapshot内を探す。Hoisted Workspaceでは典型的に`--cwd packages/<package>`付きの`../../node_modules/.bin/vitest run <test file>`になる。WrapperはDependencyを再評価・再Installし得るが、直接BinaryはSeal済みLayerに対して即座に実行される。記録されたCommand IDは、ProbeしたargvとそのままのCwdを全Batch実行で再利用する。Probeが`blocked`を返した場合は、範囲制限されたstdoutとstderrを読み、Focused CommandまたはCwdを修正し、返された`next.argv`でProbeを再実行する。検証済みCommand IDなしに`scaffold-plan`へ進まない。
