...
`scaffold-analysis`の後、`complete`を呼ぶ前に、分類済みOutcomeを要約したうえで、構造化質問ツールで保存方法を質問する。
...
Ephemeralはすべての終了経路で保証する。`complete`は最終SurfaceのRenderと既定Cleanupを同じ段階で行うため、Artifact保持は`complete`前に決定する——Render後に質問しても、もう何も保存できない。無回答はDiscardとして、その既定Cleanupを使う。明示的にローカル保存またはMarkdown出力を選んだ場合だけ`--retention keep`を使う。失敗、Timeout、中断、Abortでは同じ冪等Cleanupを直ちに呼ぶ。Cleanupできない場合は残ったPathだけを正確に報告する。
...
正準のReview Surface後に、**テストを適用**、**Patchを出力**、**破棄**の構造化質問を1件提示する。対応するOracleが未解決なら「テストを適用」は提示しない。「テストを適用」の選択を、同じCycleでApply tests modeへ進むための明示的許可として扱う。実行Artifact保存の質問は別のCheckpointである——引き渡しは`complete`後も残るが、Run Artifactは残らないため、保存は`complete`前に決定する(Run Artifactの節を参照)。無回答は破棄として扱う。
...
ローカルGitは、厳密に読み取り専用の根拠収集にだけ使う。許可するコマンドは`git diff`、`git show`、`git log`、`git rev-parse`、`git merge-base`、`git ls-files`に限定する。`git archive`は読み取り専用ではなく(`-o`がファイルを書く)、Host Gateが拒否する。Hook-host実行中は各Commandを`git --no-pager`で始め、`diff`、`show`、`log`には`--no-ext-diff --no-textconv`を付ける。Host提供のDiffまたは展開済みのBase・Head Snapshotがある場合はそちらを優先する。
...
推奨Typeを人間に確認または訂正してもらう。推奨はRoutingにだけ使い、Specification Evidenceとして扱わない。起動時にTypeが明示されている場合はOpen-endedなScope質問をせず、そのTypeを復唱して確認する。回答が得られない非対話Sessionでは、推奨Review Typeで続行し、人間未確認である旨をAnalysisへ記録する。
...
人間が、確認、訂正、仕様オーナーへ保留、またはRepository Evidenceを根拠に続行できるようにする。訂正をIntent Contract作成前に反映する。これは意味のCheckpointであり、機械操作ごとの許可ではない。非対話Sessionでは、このCheckpointを一度提示したうえでRepository Evidenceを根拠に続行し、人間未確認である旨を記録する。ただしIntent／Oracle判断はこの既定を引き継がない——未解決Oracleは対応するChallengeを必ず停止する。Run全体の人間Checkpointは、Review Type、Diff理解、Repository Evidenceで解決できない場合だけのIntent／Oracle判断、最終解釈／Dispositionの4つとする。Checkpoint間で同じ確認を繰り返さない。

### Review-onlyの必須Entry Point

Native Host IntegrationがTrusted Preflightを完了した後だけ、このWorkflowへ入る。Claude Codeでは実際に表示されるPlugin名前空間付きCommand `/socratic:socratic`を、Codexまたは対応するローカルCursor Desktop Workspaceでは`$socratic`を、いずれもPromptの先頭で実行する。Promptの他の位置での言及——引用、Fence内、文中——はInvocationではなく、Hostを起動しない。`$maieutic`と`$elenchus`の直接起動にも同じHost Gateを使う。各Host PluginはSkillの実行前にLive brokerを自動起動し、正確なPreflight Commandを注入してTool Gateを有効化する。実行中ManifestはTurn間で維持し、完了・Abort・Idle・broker stale時にSessionをCleanupする。注入されたCommandを捏造・変更しない。Standalone Skill Installと未対応のCursor CLI、Remote、Cloud Surfaceは準拠Entry Pointではない。
...
すべてのReview-only Mutation Runは、信頼されたHost Adapterと固定Runner Pipeline——`preflight`、1回の`runbook`、構造化`inspect`、`scaffold-contract`、`stage-artifact`、必要な`execute --phase prepare`、成功した`probe-command`、`scaffold-plan`、1回の`challenge-batch`、`scaffold-analysis`、`complete`——を必ず使用する。このPipelineが正準のOrchestration Contractである。MaieuticまたはElenchusの文書が異なるMutation機構を記述している場合、その記述は各Skillの単独利用にだけ適用され、Socratic起動のRunには決して適用されない。すべてのRunner Commandは同期・Foregroundで実行し完了を待つ。`prepare`、`probe-command`、`challenge-batch`、`complete`をBackground Taskとして起動したり、完了待ちのためにTurnを終了したりしない——非対話SessionではTurn終了がBackground Processを殺し、Runを無効化する。Standalone CLIはReady Runを作れず、自己申告Attestation JSONを受理しない。Host Adapter、Schema、またはHostがAttestしたRead-only/Write-monitor CapabilityがなければMutation前に`blocked`で停止する。Schema v10の`verified: true`はRunnerが信頼するHostのAttestationを受理したことを意味し、Runner自身がOS境界を独立検証したという意味ではない。手作業の近似、Primary変更後の復元、Runner外のRepository Command、Attestation Fieldや完全Reportや4ブロックの手書きを正規Runとして提示しない。

Preflight直後、Preflight結果の`next.argv`にある`runbook` Commandを正確に一度だけ実行する。runbookはID用語集(DEC/INV/FX/UNR/CMD/MUT)、Gate順序、編集可能Fieldを説明し、正確な次Commandを返す。成功した各Runner Resultは`next.argv`を持つ——`<angle-bracket>` Placeholderだけを置き換えて逐語的に実行し、引数を推測・並べ替えしない。各Scaffold Resultは`editable_fields`とRunner所有の`field_guide`(必須Key、許可Enum値、条件規則、Schema-validな要素Template)も持つ。Schemaを開いたり形を推測したりせず、このGuideを使う。最初のStepの前に、runbookのExecution Planをユーザーの言語で短い番号付きListとして提示する。各Phaseの遷移は内部Command名ではなく人間の目標(「事故Patternを検証中」)として告知し、1つのPhaseが30秒を超えるときは理由と経過時間を報告し、根拠のないPercentageを決して表示しない。

注入されたHost Review Contextを決定論的なFast Pathとして使用する。Base/Head Diff、Package Manager、固定Pipelineの再発見をSubagentへ委譲せず、Focused Testの直接Executableが判明している場合にPackage-manager Wrapperを試行錯誤しない。Intentを推論して`scaffold-contract`を呼び、返されたDocumentの意味的なPlaceholderをRepository Evidenceから置き換え、返された絶対`artifact_path`を`Write`で一度だけ新規作成する。これをStageした後だけ、Observable Oracleを示す`contract_ids`付きChallengeを投入する。RunnerはStage済みContractがないMutationを拒否し、未解決Oracleへ対応するChallengeをすべてBlockする。構造化質問を提示し、正確なProvenanceで回答を記録し、新しいRunで再Stageしてから該当Oracleを再開する。
...
Host AdapterはRun ID、Nonce、保護された外部Storage、Artifact Index、Repository全体の保護証跡を発行する。Shell探索や決定論的Subagentの代わりに、`inspect diff|file|search|tests`で範囲を制限したRead-only Evidenceを取得する。依存導入は`execute --phase prepare`で一度だけ行う。Mutation IDを割り当てる前に、正確なFocused Test argvを`probe-command`へ渡す。RunnerはFresh Source Cloneで実行し、TTY、Package Manager、cwd、Timeout、Dependencyの問題をInfrastructure FailureとしてMutation前に検出し、成功時だけ再利用可能なCommand IDを記録する。Probe成功後、Runnerは`node_modules`と認識済みPython仮想環境を専用Hash付き共有Dependency LayerとしてSealし、各MutantのHOME、Temp、Cacheは個別に保つ。Probeには`pnpm exec`や`npm run`のようなPackage-manager Wrapperではなく、直接のTest Executableを使う——まず`inspect search`でPrepared Snapshot内を探す。Hoisted Workspaceでは典型的に`--cwd packages/<package>`付きの`../../node_modules/.bin/vitest run <test file>`になる。WrapperはDependencyを再評価・再Installし得るが、直接BinaryはSeal済みLayerに対して即座に実行される。記録されたCommand IDは、ProbeしたargvとそのままのCwdを全Batch実行で再利用する。Probeが`blocked`を返した場合は、範囲制限されたstdout、stderr、および`hint`を読み、Focused CommandまたはCwdを修正し、返された`next.argv`でProbeを再実行する。検証済みCommand IDなしに`scaffold-plan`へ進まない。Sandbox CommandはProjectそのもののToolchainで実行する: 注入されたRuntime Pythonは`run_review.py`専用であり、Versionの要求が関わる場合はProject自身のInterpreterまたはTest Binaryを絶対Pathで使う。Prepared Snapshotに`.git`は含まれない——VCS版数系のBuild Backendはプリセットの`SETUPTOOLS_SCM_PRETEND_VERSION`に依存し、versioningitはProject側の`default-version`を要する——うえ、HOMEはSandbox内へRedirectされるため、HOME依存のTool Shim(uv、nvm、pyenv)は解決できないことがある。Toolは絶対Pathで呼ぶことを優先する。Infrastructure失敗の後は、ToolやArgumentを変える前に、Resultの`diagnose.argv`にあるRead-onlyな`doctor` Commandを実行する。これはSandboxのPATH、そこで解決できるToolchain実行ファイルとそのVersion、ProjectのInterpreter・Build Backend要件、VCS Metadataの状況、直近の失敗Commandを報告する——環境をAd-hocなShellで探ることは決してしない。

`challenge-plan.json`には、検証済みCommand ID、Intentに結び付いた事故Metadata、`replace-exact`または`delete-exact`のAnchor Editだけを書く。Source File全文、MutantごとのCommand argv、Hash、Ledger Dataを埋め込まない。RunnerはMutant作成前に全AnchorをPrepared Sourceへ照合し、Source SnapshotをSealしてMutation IDごとのCopy-on-write Source Cloneを作り、Seal済みDependency Layerを接続し、自身でEditを適用して、検証済みCommandを並列実行しPlan順にRaw Outcomeを返す。全Raw Outcomeが返ってから分類し、Runner所有のRaw Outcomeと推論側の解釈を分離する。

Host ContextはSession固有の`artifact_root`を1つ提供する。Agentが書けるのは`intent-contract.draft.json`、`challenge-plan.json`、`review-analysis.json`と——実験的Prototype Pathに限り——`experiment-plan.json`だけとする。正準の各文書はRunner Scaffoldから始まる: Contractのstaging前に`scaffold-contract`、成功したProbe後に`scaffold-plan`(検証済みCommand IDを束ねる)、`challenge-batch`後に`scaffold-analysis`。各Scaffold Commandは、Schema-validな`document`と絶対`artifact_path`を返すが、対象ファイルは先に作成しない。Agentは意味的なPlaceholderだけを埋め、返されたPathを`Write`で一度だけ新規作成する。作成後に修正が必要なら、必ず`Read`してから`Edit`し、既存Scaffoldへ`Write`を再試行しない。`challenge-batch`後に`scaffold-analysis`を呼ぶと、RunnerはPlanとRaw Outcomeを結合したAnalysis Documentを返す。AgentはそのIntent、分類理由、検出Test、Follow-up、レビュワー向けClaimだけを編集する。Process失敗は最初から`killed`にせず`inconclusive`で提示する。機械情報やRun Identity等を含めず、Schemaを読んだり文書をゼロから組み立てたりしない。分類後に`complete`を一度だけ呼ぶ。RunnerがSemantic Inputを検証し、Report／Review Draftを生成・Stageし、全分類をRaw Evidenceと照合し、正準SurfaceをRenderしてCleanupする。`complete`の失敗は終端であり、使い捨てRunはCleanupされる。削除済みManifestで再試行せず、失敗を報告して必要なら新しいRunを開始する。明示的にArtifact保持を選んだ場合だけ`--retention keep`を使い、それ以外は既定の`discard`を使う。
...
隔離実行、安定したBaseline、帰属可能なMutantごとの新しいSandbox——Runnerの検証済み`challenge-batch`が並行実行するのであり、共有Workspaceで編集を交互に行うことは決してない——、明示的な`not_challenged`、本番Mutationが残っていない実行後証跡を必須とする。Review-onlyで提案テストを証明した場合は、検証済みの引き渡しも必須とする。
