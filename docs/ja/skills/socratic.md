...
すべてのReview-only Mutation Runは、信頼されたHost Adapterによる`preflight_with_host`、Baseline `execute`、1回以上の検証済み`challenge-batch`（または同等の個別Mutation・Execution）、3つの検証済み`stage-artifact`、`finish`、`cleanup`を必ず使用する。Standalone CLIはReady Runを作れず、自己申告Attestation JSONを受理しない。Host Adapter、Schema、またはHostがAttestしたRead-only/Write-monitor CapabilityがなければMutation前に`blocked`で停止する。Schema v9の`verified: true`は、Runnerが信頼するHostのAttestationを受理したことを意味し、Runner自身がOS境界を独立検証したという意味ではない。手作業の近似、Primary変更後の復元、`execute`外のCommand、Attestation Fieldや完全Reportや4ブロックの手書きを正規Runとして提示しない。
