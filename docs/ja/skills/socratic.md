...
正準Surfaceは4ブロックのままとする。完了前にIntent Contract、Mutation Report、Review Objectを同梱SchemaとCross-artifact Checkで検証し、`scripts/validate_and_render.py`だけで描画する。Parse失敗、Schema失敗、未知Contract参照、安全でないReview-only Postflight、Renderer外のProseは完了をBlockedとする。証明済みテストの処理方法は5つ目のReviewブロックではなく、その後にHostの構造化質問UIで提示する。
