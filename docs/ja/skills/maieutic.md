...
判断を記録する前に[Intent Contract](maieutic-intent-contract.md)を読む。単独利用の場合に限り、Contract Artifactを同梱Schemaで検証する。SocraticにHostされたRunではSchemaファイルを決して開かない——ContractはRunnerの`scaffold-contract`が返す文書から始まり、その`editable_fields`と`field_guide`が正式な構造を伝え、Stagingが検証を行う。テスト選択時は[QA技法の選択](maieutic-qa-techniques.md)を読む。
...
未解決項目を作る前にCommit Title、Pull Request本文、Changeset、Code Commentを調べ、一貫した権威ある根拠は`repository-established`のDecisionにする。複数の合理的な期待が残り、回答が観測可能なOracleを変える場合だけ構造化質問を行う。

- 仕様Ownerまたは権限を委任された代理人の明示回答はDecisionの`user-confirmed`
- 仕様決定権のないレビュアーが評価前提を選ぶ場合は`reviewer-selected-benchmark-assumption`
...
現在のContractをWorking Tree外の一時Artifactとして管理する——単独利用では同梱Schemaで、Socratic配下ではRunnerのStaging Gateで検証する。確認のたびに更新し、PathをOrchestratorまたはElenchusへ渡す。`.socratic/`配下への書き込みは、Artifact方針でユーザーがローカル保存を選んだ場合だけ行う。その際は`.socratic/intent-contract.json`を使い、既存ファイルが別の変更を表す場合は上書きせず`.socratic/contracts/<change-id>.json`へ保存する。
...
回答者の権限が不明なら、選択肢に**保留／仕様オーナーへ確認**を含める。選択時は未解決を維持し、対応するOracleだけを停止し、仕様Owner向けのCopy-ready質問を生成する。
...
既定のReview-only modeでは、テストの実装と証明をDisposable環境だけで行い、提案テストとして報告する。Socratic配下ではMaieuticがテストを設計し、そのDisposable環境での証明と引き渡し作成はElenchusが行う。Working Treeへの適用は、ユーザーがテスト追加を明示的に依頼したApply tests modeの場合だけ行う。
