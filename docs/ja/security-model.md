...
Standalone Skill配布物は3つのSkill Directoryにある43個のUTF-8 Text Fileで構成され、そのうち16個は同梱Python Source File（4個のScript Entrypointと、`run_review.py`の背後にある12 ModuleのRunner Package）で、現行Run Schemaと実験的なPlan・Evidence・Interpretation Schemaも含みます。監査対象Multi-Host Plugin Component Setは、これらのSkillにClaude Code・Codex・CursorのManifest、Marketplace、Host Hook、共通Broker、Plugin管理Python Runtime Bootstrapを加えた合計63個のUTF-8 Text Fileです。Claude MarketplaceはRepository RootをSourceにするため、Marketplace Checkoutには`demo/`、`docs/`、`site/`など、この監査対象Runtime Component Set外のRepository Fileも実体化されます。63 Fileという記述は監査対象Plugin BundleとRelease Assetを指し、Source Checkoutに実体化される全File数ではありません。Python FileにPOSIX Execute Bitはありませんが、Python Interpreterから実行されます。CIの実行可能File拒否は監査対象Component SetについてPOSIXの`0o111` Execute-bit Maskを検査し、予期しないFile、許可されていない拡張子、Symbolic Link、Binary、未承認の外部Hostも拒否します。Release AssetにはManifestとSHA-256 Checksumが含まれます。
...
依存準備とMutation実行ではFilesystem Roleを分離します。Baseline Commandで依存を一度だけ導入し、Probe成功後にRunnerが導入済みPackageと認識済みPython仮想環境をRunner所有のDependency Layerへ移して、専用Content HashでSealします。各Node Sandboxには、導入済みEntryだけを共有LayerへLinkする浅いLocal `node_modules` Directoryを用意します。これにより`node_modules/.vite`等のRuntime出力はSandboxごとに分離され、Seal済みPackageを変更しません。各Mutation IDのHOME、Temp、Cacheも個別です。SourceのStaleness確認はSourceと設定だけを走査し、`finish`はSource Snapshotまたは導入済みPackage Contentのどちらかが変わっても独立して拒否します。Sourceには引き続きAPFS CloneまたはLinux ReflinkのCopy-on-writeを優先し、利用できない場合は`full-copy`を記録します。
...
各Live Host SessionはHost Storage配下にPrivateな`artifact_root`を1つ作成する。Write Toolがその直下へ作成できるのは、固定された3つのSemantic Input、`intent-contract.draft.json`、`challenge-plan.json`、`review-analysis.json`だけである。各InputについてRunnerはSchema-validなDocumentと絶対固定Pathを返すが、Fileを先に作成しない。Agentは1回の`Write`で新規作成でき、その後の修正には`Read`に続けて`Edit`を使わなければならない。Contractは`stage-artifact`でStrict Validationされ、Host管理Artifact IndexへCreate-once Hashを記録する。PlanとAnalysisはそれぞれのRunner境界で検証される。Report／Review Draftを作成・StageできるのはRunnerだけである。それらのDraftはRun Identity、Raw Execution Evidence、Attestation、Isolation、Postflight、Renderer Claimを指定できない。`finish`が信頼済みManifestとAppend-only Ledgerからそれらを導出し、生成したMutation Report v10とCross-artifact ReferenceをRenderer前に検証し、TerminalへはRenderer stdoutだけを書く。Primary、Disposable Sandbox、Manifest／Ledger、任意のTemporary Directory、他Repository、User設定、Plugin CodeへのPathは引き続き拒否する。成功RunのHost ArtifactはユーザーがDispositionを解決するまでだけ保持し、その後`cleanup`で削除する。終端となる`finish`失敗時はSandbox、Artifact、Index、Ledger、Manifestを即時Cleanupし、削除済みManifestで再試行してはならない。
