...
Schema v8はv7で導入したJSON Field名`verified`を維持します。Protection Evidenceの`verified: true`は、信頼するHost Adapterが発行したAttestationをRunnerが受理したことを表し、Runner自身がOS Protection境界を独立検証したという意味ではありません。Schema v8ではHost由来のRaw Execution Evidenceと推論側のOutcome Interpretationも分離します。Nonzero Exitだけで`killed`とはできず、Behavioral Assertion Failureを特定できないInfrastructure Failure、Crash、Timeout、Unparseable Outputは`inconclusive`のままです。
