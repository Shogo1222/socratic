...
Mutationでは最終状態一致だけでは不十分です。必須Run Entry PointはRepository外Sandbox作成前にHost／OS保護Attestationを要求し、全Guarded Mutation WriteをRun Ledgerへ結合してFinishで検証します。復元後に同一Byteへ戻ってもPrimary WriteがあればRunは無効です。HostがRepository全体のRead-only保護またはWrite Monitorを提供できない場合、Mutation前に`blocked`で停止します。
